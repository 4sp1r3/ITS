...
void Crossover::cross(Solution& sol1, Solution & sol2) const // dadas dos soluciones de la poblacion, las cruza
{
	...
	ret.getCoord() = (sol1.getCoord() + sol2.getCoord()) / 2.0 ;
	ret.getDecay() = (sol1.getDecay() + sol2.getDecay()) / 2.0 ;

	if (sol1.pbm().direction() == Direction::maximize) {
		if (sol1 > sol2) {
...
void Mutation::mutate(Solution & sol) const {
	...
	//mutate decays
	for (int i = 0; i < sol.pbm().getNumNodes(); i++) {
