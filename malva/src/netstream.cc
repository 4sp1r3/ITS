...
NetStream::~NetStream() {
    delete [] packin_buffer;
    delete [] packout_buffer;
    this->reset();
}
...
NetStream& NetStream::operator>>(bool& d) {
    rcv(&d, 1, NET_BOOL, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(bool d) {
    send(&d, 1, NET_BOOL, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(char& d) {
    rcv(&d, 1, NET_CHAR, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(char d) {
    send(&d, 1, NET_CHAR, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(short& d) {
    rcv(&d, 1, NET_SHORT, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(short d) {
    send(&d, 1, NET_SHORT, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(int& d) {
    rcv(&d, 1, NET_INT, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(int d) {
    send(&d, 1, NET_INT, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(long& d) {
    rcv(&d, 1, NET_LONG, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(long d) {
    send(&d, 1, NET_LONG, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(float& d) {
    rcv(&d, 1, NET_FLOAT, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(float d) {
    send(&d, 1, NET_FLOAT, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(double& d) {
    rcv(&d, 1, NET_DOUBLE, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(double d) {
    send(&d, 1, NET_DOUBLE, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(char* d) {
    rcv(d, MAX_MSG_LENGTH, NET_CHAR, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(char* d) {
    send(d, strlen(d) + 1, NET_CHAR, default_target);
    return (*this);
}
...
NetStream& NetStream::operator<<(void* d) {
    send(d, strlen((char*) d) + 1, NET_CHAR, default_target);
    return (*this);
}
// Extended data types from version 1.5 on

NetStream& NetStream::operator>>(unsigned char& d) {
    rcv(&d, MAX_MSG_LENGTH, NET_UNSIGNED_CHAR, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(unsigned char d) {
    send(&d, 1, NET_UNSIGNED_CHAR, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(unsigned short int& d) {
    rcv(&d, MAX_MSG_LENGTH, NET_UNSIGNED_SHORT, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(unsigned short int d) {
    send(&d, 1, NET_UNSIGNED_SHORT, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(unsigned int& d) {
    rcv(&d, MAX_MSG_LENGTH, NET_UNSIGNED, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(unsigned int d) {
    send(&d, 1, NET_UNSIGNED, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(unsigned long int& d) {
    rcv(&d, MAX_MSG_LENGTH, NET_UNSIGNED_LONG, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(unsigned long int d) {
    send(&d, 1, NET_UNSIGNED_LONG, default_target);
    return (*this);
}

NetStream& NetStream::operator>>(long double& d) {
    rcv(&d, MAX_MSG_LENGTH, NET_LONG_DOUBLE, default_source);
    return (*this);
}

NetStream& NetStream::operator<<(long double d) {
    send(&d, 1, NET_LONG_DOUBLE, default_target);
    return (*this);
}
// SET-GET TARGET AND SOURCE PROCESSES

NetStream& __set_target(NetStream& n, const int p) {
    return n._set_target(p);
}
...
NetStream& __get_target(NetStream& n, int* p) {
    return n._get_target(p);
}
...
NetStream& __set_source(NetStream& n, const int p) {
    return n._set_source(p);
}

NetStream& NetStream::_set_source(const int p) {
    /*assert(p>=0);*/ default_source = p;
    return (*this);
}

NetStream& __get_source(NetStream& n, int* p) {
    return n._get_source(p);
}
